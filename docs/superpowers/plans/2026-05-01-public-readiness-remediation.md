...
| Final public polish | 2026-05-02 | codex/public-readiness-final-polish | pending | `python -m pytest tests/e2e/test_smoke.py --e2e -q`; Gitleaks v8.30.1 full-history scan; `python build.py --no-clean`; `python -m ruff check src/catalyst tests scripts desktop_app.py build.py`; `python scripts/check_env_example.py`; `python scripts/check_tracked_secrets.py`; `python -m vulture src/catalyst scripts desktop_app.py build.py scripts/vulture_whitelist.py --min-confidence 90`; `python -m pip_audit -r requirements.txt -r requirements-dev.txt`; `python -m pytest tests -q` | passed | Added sanitized README screenshot from isolated E2E data; expanded third-party notices and public release checklist; set GitHub homepage to latest release URL; confirmed branch protection still returns GitHub's private-repo plan restriction; Gitleaks scanned 570 commits with no leaks found; full tests reported 2817 passed, 12 skipped, 3 warnings, 42 subtests. |
